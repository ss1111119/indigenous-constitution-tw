...
在 GitHub 的 **Actions → Refresh population data → Run workflow** 填入期別（如 `11507`）即可
略過探測直接轉該期，其餘驗證一律照常套用。用於幅度檢查擋下一次真實的大幅變動、
且已人工確認之後的放行。
...
2. 若是**來源異常**（口徑改變、欄位語意變動）：不要放行。先確認上游發生什麼事，
   必要時調整轉檔腳本，再重跑。
3. 若確認是**真實變動**：以「手動指定期別」重新觸發該期。人工確認的責任在此，
   流程不會自己放寬門檻。
4. 兩種情況都不要為了讓流程變綠而調高門檻——門檻調高一次，下一次數量級錯誤就過得去了。
